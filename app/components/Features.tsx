        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
          {simFeatures.map((feature, index) => {
            ...
            return (
              <motion.div
                key={feature.title}
                className={`group rounded-2xl border ${feature.border} bg-white/[0.04] p-6 transition-[background-color,box-shadow] duration-300 md:hover:-translate-y-1 hover:bg-white/[0.07] ${feature.glow}`}
          ...
          <motion.div
            id="report"
            className="scroll-mt-24 overflow-hidden rounded-2xl border border-white/[0.1] bg-white/[0.04] transition-[background-color,box-shadow] duration-300 md:hover:-translate-y-1 hover:shadow-[0_8px_32px_rgba(99,102,241,0.12)] sm:col-span-2"
            initial={{ opacity: 0, scale: 0.92 }}
            whileInView={{ opacity: 1, scale: 1 }}
            viewport={{ once: true, margin: "-40px" }}
